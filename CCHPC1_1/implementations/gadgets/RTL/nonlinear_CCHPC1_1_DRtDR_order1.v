module nonlinear_CCHPC1_1_DRtDR_order1 #(parameter CONF = 2'b00, aINV = 1'b0, bINV = 1'b0, zINV = 1'b0
                                       // 2'b00: and
                                       // 2'b01: nand
                                       // 2'b10: nor
                                       // 2'b11: or
    ...
    mtg_opt #(.invT3(0)) mtg_L1_inst (.a(in_a[0]), .b(in_b[0]), .r(randL1), .t_in(t_in_layer1[7:0]));

    REG_prch_wNOR #(.WIDTH(8)) reg_L1_inst (clk, prch[0], t_in_layer1[7:0], t_out_layer1[7:0]);

    // select    
    op_opt op_t_inst10 (.a(in_a[2:1]), .b(in_b[2:1]), .t_out(t_out_layer1[3:0]), .z(z[1]));
    op_opt op_f_inst10 (.a(in_a[2:1]), .b(in_b[2:1]), .t_out(t_out_layer1[7:4]), .z(z[2]));
