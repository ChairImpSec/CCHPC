...
module nonlinear_CCHPC1_1_generic_DRtSR_duality #(
    ...
    clk, prch0, prch1, a_layer0, a_duality0, a_duality1, b_layer0, b_duality0, b_duality1, r_SR, z_SR
);
    ...
    wire [2*d*(d-1)-1:0] t_SR0;  // pre-charged single-rail for duality instance 0
    wire [2*d*(d-1)-1:0] t_SR1;  // pre-charged single-rail for duality instance 1
    ...
    generate
        ...
        for (tp = 1; tp < d; tp = tp+1) begin : gen_layer_
            ...
            REG_prch_wNOR #(
                ...
                .z    (t_SR0[SR_OFFSET+SR_WIDTH-1:SR_OFFSET])
            );
            ...
            REG_prch_wNOR #(
                ...
                .prch (prch1[tp-1]),
                .a    (t[SR_OFFSET+SR_WIDTH-1:SR_OFFSET]),
                .z    (t_SR1[SR_OFFSET+SR_WIDTH-1:SR_OFFSET])
            );
    ...
    nonlinear_CCHPC1_1_generic_DRtSR_duality_consecutive #(
        .security_order(security_order),    // security order
        .FORWARD_R(FORWARD_R),							// forward layer output
