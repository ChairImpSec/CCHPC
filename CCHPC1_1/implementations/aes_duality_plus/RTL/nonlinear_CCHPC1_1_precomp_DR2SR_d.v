module nonlinear_CCHPC1_1_precomp_DR2SR_d #( parameter security_order = 1, CONF = 2'b00
                                                                        // 2'b00: and
                                                                        // 2'b01: nand
                                                                        // 2'b10: nor
                                                                        // 2'b11: or
    ...
    generate
        ...
        for (i = 1; i < (d-1); i=i+1) begin : loop_delay_
            ...
            REG_pipeline_vec #(.depth(d-i-2)) reg_pipeline_a1_inst (clk, a[i], a_delayed[i][d-i-2:0]);   
            REG_pipeline_vec #(.depth(d-i-2)) reg_pipeline_b1_inst (clk, b[i], b_delayed[i][d-i-2:0]);
    ...
    generate
        ...
        if (d == 2) begin : first_order
            ...
            mtg_opt_t_only #(.invT3(0)) mtg_L1_inst (.a(a_delayed[0][0]), .b(b_delayed[0][0]), .r(randL1), .t_in(t_in[3:0]));
        ...
        if (d == 3) begin : second_order
            ...
            mtg_opt_t_only #(.invT3(0)) mtg_L2_inst0 (.a(a_delayed[0][1]), .b(b_delayed[0][1]), .r(randL20), .t_in(t_in[ 7:4]));
            mtg_opt_t_only #(.invT3(1)) mtg_L2_inst1 (.a(a_delayed[1][0]), .b(b_delayed[1][0]), .r(randL21), .t_in(t_in[11:8]));
        ...
        if (d == 4) begin : third_order
            ...
            mtg_opt_t_only #(.invT3(0)) mtg_L3_inst0 (.a(a_delayed[0][2]), .b(b_delayed[0][2]), .r(randL30), .t_in(t_in[15:12]));
            mtg_opt_t_only #(.invT3(1)) mtg_L3_inst1 (.a(a_delayed[1][1]), .b(b_delayed[1][1]), .r(randL31), .t_in(t_in[19:16]));
            mtg_opt_t_only #(.invT3(1)) mtg_L3_inst2 (.a(a_delayed[2][0]), .b(b_delayed[2][0]), .r(randL32), .t_in(t_in[23:20]));
